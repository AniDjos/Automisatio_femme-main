...
@section('content')
        ...
        <div class="recherche">
            <form action="{{ route('groupements.index') }}" method="GET" class="recherche-groupement">
                <div class="search-wrapper">
                    ...
                    <input type="text" id="search-groupement" name="search" value="{{ request('search') }}"
                        placeholder="Rechercher un groupement par son nom">
                </div>
                ...
                <div class="filter-wrapper">
                        ...
                        <select name="departement" id="departement" class="filter-select">
                            ...
                            @foreach ($departements as $departement)
                                <option value="{{ $departement->departement_id }}"
                                    {{ request('departement') == $departement->departement_id ? 'selected' : '' }}>
                        ...
                        <select name="commune" id="commune" class="filter-select">
                            ...
                            @foreach ($communes as $commune)
                                <option value="{{ $commune->commune_id }}"
                                    {{ request('commune') == $commune->commune_id ? 'selected' : '' }}>
                    ...
                    <div class="select-container">
                        <i class="fas fa-map filter-icon"></i>
                        <select name="arrondissement" id="arrondissement" class="filter-select">
                            ...
                            @foreach ($arrondissements as $arrondissement)
                                <option value="{{ $arrondissement->arrondissement_id }}"
                                    {{ request('arrondissement') == $arrondissement->arrondissement_id ? 'selected' : '' }}>
                ...
                <div class="container-groupement">
                    ...
                    <div class="groupement-info">
                        <p class="info-item" data-tooltip="Activité principale du groupement">
                            ...
                            <span>Activité principale : <strong>{{ $groupement->activite_principale_nom ?? 'Non spécifiée' }}</strong></span>
                        </p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const searchInput = document.getElementById('search-groupement');
            const containerPrin = document.querySelector('.container-prin');
            const groupements = containerPrin.querySelectorAll('.container-groupement');

            searchInput.addEventListener('keyup', function () {
                const query = searchInput.value.toLowerCase().trim();

                groupements.forEach(groupement => {
                    const groupementName = groupement.querySelector('.title').textContent.toLowerCase();

                    // Vérifier si le nom du groupement contient la requête
                    if (groupementName.includes(query)) {
                        groupement.style.display = 'block'; // Afficher le groupement
                    } else {
                ...
                const visibleGroupements = Array.from(groupements).filter(groupement => groupement.style.display === 'block');
                if (visibleGroupements.length === 0) {
                    if (!document.querySelector('.empty-message')) {
                        const noResultsMessage = document.createElement('div');
                        noResultsMessage.className = 'empty-message';
                        noResultsMessage.textContent = 'Aucun groupement trouvé.';
                        containerPrin.appendChild(noResultsMessage);
                    }
                } else {
